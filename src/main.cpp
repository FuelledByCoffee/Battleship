...
int main() {
	...
	keypad(stdscr, true); // allows for testing against predefined inputs such
	                      // as KEY_UP
	...
	do {
		...
		int choice = menu.MenuPicker(); // returns 0 as there is only 1 menu
		                                // option
		if (choice == game) {
			std::string gameType = "normal"; // stretch: add gameTypes
			menu.StartGame(gameType);
		}
		exit = menu.CloseProgram(); // asks the user if they would like ot play
		                            // again
