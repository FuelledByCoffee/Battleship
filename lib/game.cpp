	...
	*/
	...
	do {
		do {
			...
			oppPlayer->attacked(y - 97, x - 48); // initializes an attack
			                                     // subtract because y and x are
			                                     // chars
			...
			// checks if there is a winner, if there is, ends game and returns
			// winner
			...
			currPlayer = nullptr; // stops pointing so that current player can
			                      // switch
			oppPlayer = nullptr;
...
void Game::initPlayers() {
	...
	// name mvprintw(1, 1, "Player 2 is %s", getName(1)); refresh(); getch();
	clear();
}
