...
// probably work more efficiently
void Board::displayUserBoard() const { //       if "board" was a pointer that
...
// this function also returns board, but removes sight of ships that have not
// been hit
void Board::displayOpponentBoard() const {
	for (int i = 0; i < 10; ++i) {     // rows : y
		for (int j = 0; j < 10; ++j) { // columns : x
			if (board[i][j] == '#') {
				// std::cout << ". ";
				mvprintw(i + 2, j * 2 + 26, "."); // x is +22 from the user
				                                  // board
			} else {
				// std::cout << board[i][j] << " ";
...
// In board function which checks that an input leads to a spot in the board &
// checks for ship overlays
bool Board::inBoard(int i, int j, char direction, char symbol) {
	if ((direction == 'w') && (boardCopy[i][j] == symbol)
	    && (((i - 1) < 0) || (boardCopy[i - 1][j] == '#'))) {
		return false;
	} else if ((direction == 'a') && (boardCopy[i][j] == symbol)
	           && (((j - 1) < 0) || (boardCopy[i][j - 1] == '#'))) {
		return false;
	} else if ((direction == 's') && (boardCopy[i][j] == symbol)
	           && (((i + 1) > 9) || (boardCopy[i + 1][j] == '#'))) {
		return false;
	} else if ((direction == 'd') && (boardCopy[i][j] == symbol)
	           && (((j + 1) > 9) || (boardCopy[i][j + 1] == '#'))) {
		return false;
	} else {
		return true;
	}
